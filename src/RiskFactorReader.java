...
public class RiskFactorReader {
	...
	try {
		...
		// Before hard-coding, eg: riskfactorToRangeDeceased.put("Age",
		// getRange(rowElements[5]));
		// Age is row 1
		if (row == 1) {
	    ...
	    e.printStackTrace();
	    System.out.println(
		    "'riskFactors.csv' not found. Please ensure that you have saved it in this project subfold of the eclipse-workspace");
	} catch (Exception e) {
	...
	try {
	    range[0] = new Double(Double.parseDouble(min));
	...
	try {
	    range[1] = new Double(Double.parseDouble(max));
	} catch (NumberFormatException e) {
	}
    ...
    /**
     * Calculates attributable risk of a given risk factor. We will use this to
     * extrapolate binary risk factors, given that there is not a range of values
     * that we can read in. a = num deceased, with feature; b = num recovered, with
     * feature c = num deceased, without feature; d = num recovered, without feature
     * Attributable risk = (A / A + B) - (C / C + D) In order words: (outcome risk
     * among those with feature - outcome risk among those without feature)
     * 
    ...
    /**
     * Calculates relative risk. Relative risk = (A / A + B) / (C / C + D)
     * 
     * @param a
     * @param b
     * @param c
